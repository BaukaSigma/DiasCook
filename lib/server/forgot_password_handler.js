...
const validatePassword = (password) => {
    ...
    return isLongEnough && hasUpper && hasLower && hasDigit && hasSpecial && hasNoSpaces;
};

async function sendResetCode(email) {
    if (!email) {
        return { status: 400, error: 'Электрондық пошта талап етіледі.' };
    }
...
function verifyResetCode(email, code) {
    if (!email || !code) {
        return { status: 400, error: 'Электрондық пошта және код талап етіледі.' };
    }
...
async function resetPassword(email, newPassword) {
    if (!email || !newPassword) {
        return { status: 400, error: 'Электрондық пошта және жаңа құпия сөз талап етіледі.' };
    }
